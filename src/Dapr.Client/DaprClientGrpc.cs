...
namespace Dapr.Client
{
    ...
    internal class DaprClientGrpc : DaprClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var invokeResponse = await this.MakeInvokeRequestAsyncWithResponse<TRequest, TResponse>(appId, methodName, serializedData, httpExtension, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                invokeResponse = await this.MakeInvokeRequestAsyncWithResponse<TRequest, TResponse>(request.AppId, request.MethodName, serializedData, request.HttpExtension, cancellationToken);
        ...
        private async Task<ServiceInvocationResponse<TRequest, TResponse>> MakeInvokeRequestAsyncWithResponse<TRequest, TResponse>(
            ...
            CancellationToken cancellationToken = default)
        {
            var grpcCall = MakeInvokeRequestAsync(appId, methodName, data, httpExtension, cancellationToken);
            ...
            var invokeResponse = new ServiceInvocationResponse<TRequest, TResponse>
            {
                Body = response.Data.Value.IsEmpty ? default : TypeConverters.FromAny<TResponse>(response.Data, this.jsonSerializerOptions),
                ContentType = response.ContentType,
                Headers = grpcCall.ResponseHeadersAsync.Result.ToDictionary(kv => kv.Key, kv => kv.ValueBytes),
                Trailers = grpcCall.GetTrailers().ToDictionary(kv => kv.Key, kv => kv.ValueBytes),
            };

            if (response.ContentType.Contains("grpc"))
            {
            ...
            {
                invokeResponse.HttpStatusCode = ToHttpStatusCode(grpcStatus.StatusCode);
